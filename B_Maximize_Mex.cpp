...
void solve()
{
    int n = 0, q = 0, m = 0, k = 0, l = 0, r = 0, h = 0, cnt = 0, sum = 0, mn = INF, mx = -INF, ans = 0;
   
    cin >> n >> k;
    map<int, priority_queue<int, std::vector<int>, std::greater<int>>> mp;
    ...
    int i = 0;
    for(; i < n + 20; i++) {
        if(mp.find(i%k) != mp.end() && sz(mp[i %k]) > 0 && mp[i % k].top() <= i) {
            mp[i % k].pop();
        } else {
            cout << i << endl;
            return;
